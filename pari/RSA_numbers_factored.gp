...
*/  
    ...
    for(i=1,#rsa_,r=rsa_[i];
        ...
        \\ unfactored RSA-270 after factored RSA-896, although a bit smaller
        if(i>1 && rsa_[i][1]!=270,
            assert(n>rsa_[i-1][2]));
...
\\  """
    ...
    assert(len(self.factored())==27);
    assert(len(self.factored_2())==27);
